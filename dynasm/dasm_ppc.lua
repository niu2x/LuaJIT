...
local function parse_imm(imm, bits, shift, scale, signed)
    ...
    elseif match(imm, "^[rfv]([1-3]?[0-9])$") or match(imm, "^vs([1-6]?[0-9])$")
        or match(imm, "^([%w_]+):(r[1-3]?[0-9])$") then
        werror("expected immediate operand, got register")
    else
        waction(
            "IMM", (signed and 32768 or 0) + scale * 1024 + bits * 32 + shift,
            imm
        )
        return 0
    end
...
local function parse_shiftmask(imm, isshift)
    ...
    if n then
        if shr(n, 6) == 0 then
            local lsb = band(n, 31)
            local msb = n - lsb
            return isshift and (shl(lsb, 11) + shr(msb, 4))
                       or (shl(lsb, 6) + msb)
        end
        werror("out of range immediate `" .. imm .. "'")
    elseif match(imm, "^r([1-3]?[0-9])$")
        or match(imm, "^([%w_]+):(r[1-3]?[0-9])$") then
        werror("expected immediate operand, got register")
    else
...
local function parse_u5disp(disp, scale)
    ...
    if reg and tailr ~= "" then
        ...
        if tp then
            waction(
                "IMM", scale * 1024 + 5 * 32 + 11, format(tp.ctypefmt, tailr)
            )
            return shl(r, 16)
        end
    end
