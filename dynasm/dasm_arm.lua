...
local function parse_vrlist(reglist)
    local ta, ra, tb, rb = match(
        reglist, "^{%s*([sd])([0-9]+)%s*%-%s*([sd])([0-9]+)%s*}$"
    )
...
local function parse_imm_load(imm, ext)
    ...
    if n then
        ...
        waction(
            ext and "IMML8" or "IMML12", 32768 + shl(ext and 8 or 12, 5), imm
        )
        return 0
    end
end
    ...
    if not p1 then
        if not p2 then
            ...
            if reg and tailr ~= "" then
                ...
                if tp then
                    waction(
                        ext and "IMML8" or "IMML12",
                        32768 + 32 * (ext and 8 or 12),
                        format(tp.ctypefmt, tailr)
                    )
                    return op + shl(d, 16) + 0x01000000
