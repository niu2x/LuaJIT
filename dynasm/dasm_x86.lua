...
local map_opsizenum = { b = 1, w = 2, d = 4, q = 8, o = 16, y = 32, t = 10 }
...
local function wputmrmsib(t, imark, s, vsreg, psz, sk)
    ...
    if xreg or band(reg, 7) == 4 then
        ...
        if (imark == "I" and (m == 1 or m == 2))
            or (m == nil and (vsreg or vreg)) then
            waction("MARK")
        end
        wvreg("modrm.reg", vsreg, psz + 1, sk, vreg)
                ...
                if tailr == "" then
                    ...
                    if t.opsize == "f" then
                        ...
                        if reg == "@w4" or (x64 and reg == "@d4") then
                            wwarn(
                                "bad idea, try again with `"
                                    .. (x64 and "rsp'" or "esp'")
                            )
                        end
                        t.mode = t.reg == 0 and "rmR"
                                     or (reg == "@b1" and "rmC" or "rm")
                    end
...
for name, n in pairs {
    ...
    if n == 2 or n == 3 then
        map_op[fn .. "_2"] = format(
            "Fff:D8%02XR|Fx2d:D8%XM|Fx2q:nDC%XM", nc, n, n
        )
    else
        map_op[fn .. "_2"] = format(
            "Fff:D8%02XR|fFf:DC%02Xr|Fx2d:D8%XM|Fx2q:nDC%XM", nc, nr, n, n
        )
...
local function dopattern(pat, args, sz, op, needrex)
    ...
    for c in gmatch(pat .. "|", ".") do
        ...
        elseif c == "m" or c == "M" then -- Encode ModRM/SIB.
            ...
            local psz, sk = wputop(
                szov, opcode, rex, vex, s < 0, t.vreg or t.vxreg
            )
            opcode = nil
        ...
        elseif map_vexarg[c] ~= nil then -- Encode using VEX prefix
            ...
            if b ~= 0x0f then
                werror(
                    "expected `0F', `0F38', or `0F3A' to precede `" .. c
