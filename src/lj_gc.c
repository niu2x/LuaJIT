...
static void gc_mark(global_State* g, GCobj* o)
{
    ...
    if (LJ_UNLIKELY(gct == ~LJ_TUDATA)) {
        ...
        lj_assertG(gct == ~LJ_TFUNC || gct == ~LJ_TTAB || gct == ~LJ_TTHREAD || gct == ~LJ_TPROTO
                       || gct == ~LJ_TTRACE,
                   "bad GC type %d",
                   gct);
...
static void gc_mark_uv(global_State* g)
{
    ...
    for (uv = uvnext(&g->uvhead); uv != &g->uvhead; uv = uvnext(uv)) {
        lj_assertG(uvprev(uvnext(uv)) == uv && uvnext(uvprev(uv)) == uv, "broken upvalue chain");
...
static const GCFreeFunc gc_freefunc[] = { (GCFreeFunc)lj_str_free,   (GCFreeFunc)lj_func_freeuv,
                                          (GCFreeFunc)lj_state_free, (GCFreeFunc)lj_func_freeproto,
                                          (GCFreeFunc)lj_func_free,  (GCFreeFunc)0,
#if LJ_HASFFI
...
static GCRef* gc_sweep(global_State* g, GCRef* p, uint32_t lim)
{
    ...
    while ((o = gcref(*p)) != NULL && lim-- > 0) {
        ...
        if (((o->gch.marked ^ LJ_GC_WHITES) & ow)) { /* Black or current white? */
            lj_assertG(!isdead(g, o) || (o->gch.marked & LJ_GC_FIXED), "sweep of undead object");
            makewhite(g, o); /* Value is alive, change to the current white. */
            p = &o->gch.nextgc;
        } else { /* Otherwise value is dead, free it. */
            lj_assertG(isdead(g, o) || ow == LJ_GC_SFIXED, "sweep of unlive object");
...
static void gc_sweepstr(global_State* g, GCRef* chain)
{
    ...
    while ((o = gcref(*p)) != NULL) {
        if (((o->gch.marked ^ LJ_GC_WHITES) & ow)) { /* Black or current white? */
            lj_assertG(!isdead(g, o) || (o->gch.marked & LJ_GC_FIXED), "sweep of undead string");
            makewhite(g, o); /* String is alive, change to the current white. */
            p = &o->gch.nextgc;
        } else { /* Otherwise string is dead, free it. */
            lj_assertG(isdead(g, o) || ow == LJ_GC_SFIXED, "sweep of unlive string");
            setgcrefr(*p, o->gch.nextgc);
