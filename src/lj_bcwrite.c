...
static void bcwrite_ktabk_heap_insert(TValue** heap, MSize idx, MSize end, TValue* key)
{
    MSize child;
    ...
    for (i = 0; i < sizekgc; i++, kr++) {
        ...
        if (o->gch.gct == ~LJ_TSTR) {
            ...
            if (id == CTID_INT64) {
                ...
                lj_assertBCW(id == CTID_COMPLEX_DOUBLE, "bad cdata constant CTID %d", id);
                tp = BCDUMP_KGC_COMPLEX;
            }
#endif
        } else {
            lj_assertBCW(o->gch.gct == ~LJ_TTAB, "bad constant GC type %d", o->gch.gct);
            tp   = BCDUMP_KGC_TAB;
            need = 1 + 2 * 5;
        }
        /* Write constant type. */
...
static void bcwrite_header(BCWriteCtx* ctx)
{
    ...
    *p++                  = BCDUMP_HEAD3;
    *p++                  = BCDUMP_VERSION;
    *p++                  = (ctx->flags & (BCDUMP_F_STRIP | BCDUMP_F_FR2)) + LJ_BE * BCDUMP_F_BE
...
int lj_bcwrite(lua_State* L, GCproto* pt, lua_Writer writer, void* data, uint32_t flags)
{
    BCWriteCtx ctx;
    int        status;
    ctx.pt     = pt;
    ...
    ctx.wdata  = data;
    ctx.heapsz = 0;
    if ((bc_op(proto_bc(pt)[0]) != BC_NOT) == LJ_FR2)
        flags |= BCDUMP_F_FR2;
    ctx.flags  = flags;
    ctx.status = 0;
#ifdef LUA_USE_ASSERT
