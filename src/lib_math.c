...
static void random_seed(PRNGState* rs, double d)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (u.u64 < m)
            u.u64 += m; /* Ensure k[i] MSB of u[i] are non-zero. */
        rs->u[i] = u.u64;
    }
    for (i = 0; i < 10; i++)
        (void)lj_prng_u64(rs);
 ...
 int luaopen_math(lua_State* L)
{
    ...
    LJ_LIB_REG(L, LUA_MATH_LIB_NAME, math);
    return 1;
}
