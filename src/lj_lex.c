...
static void lex_number(LexState* ls, TValue* tv)
{
    ...
    lex_save(ls, '\0');
    fmt = lj_strscan_scan((const uint8_t*)ls->sb.b,
                          sbuflen(&ls->sb) - 1,
                          tv,
    ...
    if (LJ_DUALNUM && fmt == STRSCAN_INT) {
        ...
        lj_assertLS(fmt == STRSCAN_ERROR, "unexpected number format %d", fmt);
...
static void lex_string(LexState* ls, TValue* tv)
{
    ...
    setstrV(ls->L, tv, lj_parse_keepstr(ls, ls->sb.b + 1, sbuflen(&ls->sb) - 2));
}
