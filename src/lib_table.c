...
LJLIB_REC(.)
{
    ...
    int32_t e   = (L->base + 3 < L->top && !tvisnil(L->base + 3)) ? lj_lib_checkint(L, 4)
...
{
    LJ_LIB_REG(L, LUA_TAB_LIB_NAME, table);
#if LJ_52
    lua_getglobal(L, "unpack");
    lua_setfield(L, -2, "unpack");
#endif
    lj_lib_prereg(L, LUA_TAB_LIB_NAME ".new", luaopen_table_new, tabV(L->top - 1));
    lj_lib_prereg(L, LUA_TAB_LIB_NAME ".clear", luaopen_table_clear, tabV(L->top - 1));
    return 1;
}
