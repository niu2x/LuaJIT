...
static const char*
sym_decorate(BuildCtx* ctx, const char* prefix, const char* suffix)
{
    char name[256];
    char* p;
#if LJ_64
...
static void
sym_insert(BuildCtx* ctx, int32_t ofs, const char* prefix, const char* suffix)
{
...
static int build_code(BuildCtx* ctx)
{
    ...
    ctx->sym
        = (BuildSym*)malloc((ctx->npc + ctx->nglob + 1) * sizeof(BuildSym));
    ctx->nsym = 0;
    ...
    for (i = 0; i < ctx->nglob; i++) {
        ...
        if (!(len >= 2 && gl[len - 2] == '_' && gl[len - 1] == 'Z'))
            sym_insert(
                ctx,
                (int32_t)((uint8_t*)(ctx->glob[i]) - ctx->code),
                LABEL_PREFIX,
                globnames[i]);
    }
...
static void emit_vmdef(BuildCtx* ctx)
{
    ...
    fprintf(
        ctx->fp,
        "assert(require(\"jit\").version == \"%s\", \"LuaJIT core/library "
        "version mismatch\")\n\n",
        LUAJIT_VERSION);
