...
static void
emit_asm_reloc_mach(BuildCtx* ctx, uint8_t* cp, int n, const char* sym)
{
...
static void
emit_asm_wordreloc(BuildCtx* ctx, uint8_t* p, int n, const char* sym)
{
    uint32_t ins;
    ...
    if ((ins & 0xff000000u) == 0xfa000000u) {
        ...
        fprintf(
            ...
            &"eqnecsccmiplvsvchilsgeltgtle"[2 * (ins >> 28)],
            sym);
    } else {
        fprintf(
            stderr,
...
static void
emit_asm_label(BuildCtx* ctx, const char* name, int size, int isfunc)
{
    switch (ctx->mode) {
        case BUILD_elfasm:
#if LJ_TARGET_PS3
...
void emit_asm(BuildCtx* ctx)
{
    ...
    switch (ctx->mode) {
        case BUILD_elfasm:
            ...
            fprintf(
                ctx->fp,
                "\t.section .note.GNU-stack,\"\"," ELFASM_PX "progbits\n");
#endif
#if LJ_TARGET_PPCSPE
            /* Soft-float ABI + SPE. */
