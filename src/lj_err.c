                               ...
                               UndocumentedDispatcherContext* dispatch)
{
    ...
    int        errcode = LJ_EXCODE_CHECK(rec->ExceptionCode) ? LJ_EXCODE_ERRCODE(rec->ExceptionCode)
                                                             : LUA_ERRRUN;
    if ((rec->ExceptionFlags & 6)) { /* EH_UNWINDING|EH_EXIT_UNWIND */
        ...
        if (cf2) { /* We catch it, so start unwinding the upper frames. */
            ...
            */
            lj_vm_rtlunwind(cf,
                            (void*)rec,
                            (cframe_unwind_ff(cf2) && errcode != LUA_YIELD) ? (void*)lj_vm_unwind_ff
                                                                            : (void*)lj_vm_unwind_c,
                            errcode);
...
void lj_err_verify(void)
{
    ...
    lj_assertX(_Unwind_Find_FDE((void*)lj_err_throw, &ehb),
    ...
    lj_assertX(_Unwind_Find_FDE((void *)_Unwind_RaiseException, &ehb), "broken build: external frame
    unwinding enabled, but system libraries have no unwind tables");
    */
}
        #endif
...
LJ_FUNCA int lj_err_unwind_arm(int state, _Unwind_Control_Block* ucb, _Unwind_Context* ctx)
{
    ...
    switch ((state & _US_ACTION_MASK)) {
        ...
        case _US_UNWIND_FRAME_STARTING:
            ...
            _Unwind_SetGR(ctx,
                          2,
                          cframe_unwind_ff(cf) ? (uint32_t)lj_vm_unwind_ff_eh
...
{
    if (_Unwind_GetGR(ctx, 0) == 0xff33aa88) {
        *got = 2;
    } else if (*got == 0) {
        *got = 1;
        _Unwind_SetGR(ctx, 0, 0xff33aa77);
    }
    return _URC_OK;
}
...
/* Lexer error. */
LJ_NOINLINE void
lj_err_lex(lua_State* L, GCstr* src, const char* tok, BCLine line, ErrMsg em, va_list argp)
{
    char        buff[LUA_ID_SIZE];
    const char* msg;
