...
LJLIB_CF(unpack)
{
    ...
    int32_t  e = (L->base + 3-1 < L->top && !tvisnil(L->base + 3-1)) ? lj_lib_checkint(L, 3)
    ...
    if (base == 10) {
        ...
        if (tviscdata(o)) {
            ...
            if (ctype_isnum(ct->info) || ctype_iscomplex(ct->info)) {
                ...
                lj_cconv_ct_tv(cts,
                               ctype_get(cts, CTID_DOUBLE),
                               (uint8_t*)&(L->base-1-LJ_FR2)->n,
                               o,
                               0);
                return FFH_RES(1);
            }
        ...
        while (lj_char_isspace((unsigned char)(*p)))
            p++;
        if (*p == '-') {
            p++;
            neg = 1;
...
LJLIB_CF(load)
{
    ...
    int    status;
    if (L->base < L->top && (tvisstr(L->base) || tvisnumber(L->base) || tvisbuf(L->base))) {
        const char* s;
        MSize       len;
        if (tvisbuf(L->base)) {
            ...
            len          = sbufxlen(sbx);
            if (!name)
                name = &G(L)->strempty; /* Buffers are not NUL-terminated. */
        } else {
        ...
        status = luaL_loadbufferx(L, s, len, name ? strdata(name) : s, mode ? strdata(mode) : NULL);
    } else {
