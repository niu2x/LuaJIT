...
local map_loadc = {
    ...
    [10] = {
        ...
        },
        {
            ...
            _ = { shift = 21, mask = 1, [0] = "vldrFdl", "vldmdbFNdr" },
        },
    },
    [11] = {
        shift = 20,
        ...
        },
        {
            ...
            _ = { shift = 21, mask = 1, [0] = "vldrGdl", "vldmdbGNdr" },
        },
    },
    _ = {
        shift = 0,
        mask = 0, -- NYI ldc, mcrr, mrrc.
    },
}
...
local map_load1 = { shift = 4, mask = 1, [0] = map_load, map_media }
...
local map_loadm = {
    ...
    [0] = {
        ...
        [0] = "stmdaNR",
        "stmNR",
        { shift = 16, mask = 63, [45] = "pushR", _ = "stmdbNR" },
        "stmibNR",
    },
    {
        shift = 23,
...
local function fmtload(ctx, op, pos)
    ...
    elseif ext and band(op, 0x00400000) ~= 0 then
        ...
        elseif band(op, 0xfe0) == 0x60 then
            ...
            ofs = format(
                "%s, %s #%d", ofs, map_shift[band(rshift(op, 5), 3)], sh
            )
        end
        if band(op, 0x00800000) == 0 then
            ofs = "-" .. ofs
...
local function fmtvr(op, vr, sh0, sh1)
    if vr == "s" then
        return format(
            "s%d", 2 * band(rshift(op, sh0), 15) + band(rshift(op, sh1), 1)
        )
    else
        return format(
            "d%d", band(rshift(op, sh0), 15) + band(rshift(op, sh1 - 4), 16)
        )
    end
end
...
local function disass_ins(ctx)
    ...
    for p in gmatch(pat, ".") do
        ...
        elseif p == "K" then
            x =
                "#0x" .. tohex(band(rshift(op, 4), 0x0000fff0) + band(op, 15), 4)
        elseif p == "s" then
