...
local map_movwi = { -- Move wide immediate.
    ...
    { shift = 29, mask = 3, [0] = "movnDWRg", false, "movz|movDYRg", "movkDWRg" },
}
...
local map_data3s = { -- Data processing, 3 sources.
    ...
    [0] = {
        shift = 21,
        mask = 7,
        [0] = { shift = 15, mask = 1, [0] = "madd|mulDNMA0g", "msub|mnegDNMA0g" },
    },
    false,
    false,
    false,
...
local map_lsp = { -- Load/store register pair, offset.
    ...
    [0] = {
        shift = 30,
        mask = 3,
        [0] = { shift = 26, mask = 1, [0] = "stpDzAzwP", "stpDzAzsP" },
    ...
    },
    {
        shift = 30,
        mask = 3,
        [0] = { shift = 26, mask = 1, [0] = "ldpDzAzwP", "ldpDzAzsP" },
...
local function putop(ctx, text, operands)
    ...
    if ctx.hexdump > 0 then
        ctx.out(
            format(
                "%08x  %s  %-5s %s%s\n", ctx.addr + pos, tohex(ctx.op), text,
                concat(operands, ", "), extra
            )
        )
    else
        ctx.out(
...
local function decode_imm13(op)
    ...
    if band(op, 0x00400000) == 0 then
        ...
        if immr ~= 0 then
            ...
            local x = immr == 32 and 0
                          or band(bxor(lo, hi), lshift(-1, 32 - immr))
...
local function disass_ins(ctx)
    ...
    for p in gmatch(pat, ".") do
        ...
        elseif p == "I" then
            ...
            if altname == "mov" and shf == 0 and imm12 == 0
                and (rn == 31 or rd == 31) then
                name = altname
                x = nil
            elseif shf == 0 then
                x = imm12
        ...
        elseif p == "X" then
            ...
            if opt == 2 + band(rshift(op, 31), 1)
                and band(rshift(op, second0 and 5 or 0), 31) == 31 then
                if x == 0 then
                    x = nil
                else
                    x = "lsl #" .. x
        ...
        elseif p == "F" then
            x = parse_fpimm8(op)
        elseif p == "g" or p == "f" or p == "x" or p == "w" or p == "d" or p
            == "s" then
