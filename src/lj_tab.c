...
GCtab* LJ_FASTCALL lj_tab_dup(lua_State* L, const GCtab* kt)
{
    ...
    if (hmask > 0) {
        ...
        for (i = 0; i <= hmask; i++) {
            ...
            n->key = kn->key;
            if (tvistab(&n->val))
                setnilV(&n->val); /* Replace nil value marker. */
...
void lj_tab_resize(lua_State* L, GCtab* t, uint32_t asize, uint32_t hbits)
{
    ...
    if (asize < oldasize) { /* Array part shrinks? */
        ...
        if (LJ_MAX_COLOSIZE != 0 && t->colo <= 0)
            setmref(t->array,
                    lj_mem_realloc(L, array, oldasize * sizeof(TValue), asize * sizeof(TValue)));
    }
...
TValue* lj_tab_newkey(lua_State* L, GCtab* t, cTValue* key)
{
    ...
    if (!tvisnil(&n->val) || t->hmask == 0) {
        ...
        lj_assertL(freenode >= nodebase && freenode <= nodebase + t->hmask + 1, "bad freenode");
        do {
...
LJ_NOINLINE static MSize tab_len_slow(GCtab* t, size_t hi)
{
    ...
    while ((tv = lj_tab_getint(t, (int32_t)hi)) && !tvisnil(tv)) {
        ...
        if (hi > (size_t)(0x7fffffff - 2)) { /* Punt and do a linear search. */
            lo = 1;
            while ((tv = lj_tab_getint(t, (int32_t)lo)) && !tvisnil(tv))
                lo++;
            return (MSize)(lo - 1);
        }
    }
