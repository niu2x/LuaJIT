...
static void bcemit_arith(FuncState* fs, BinOpr opr, ExpDesc* e1, ExpDesc* e2)
{
    ...
    if (opr == OPR_POW) {
        ...
        lj_assertFS(expr_isnumk(e1) || e1->k == VNONRELOC, "bad expr type %d", e1->k);
        expr_toval(fs, e1);
    ...
    if (op <= OPR_POW) {
        ...
        *e1 = *e2;
        ...
        if (e2->k == VRELOCABLE && bc_op(*bcptr(fs, e2)) == BC_CAT) {
            lj_assertFS(e1->u.s.info == bc_b(*bcptr(fs, e2)) - 1, "bad CAT stack layout");
            expr_free(fs, e1);
        ...
        lj_assertFS(op == OPR_NE || op == OPR_EQ || op == OPR_LT || op == OPR_GE || op == OPR_LE
                        || op == OPR_GT,
                    "bad binop %d",
                    op);
...
static void var_new(LexState* ls, BCReg n, GCstr* name)
{
    ...
    lj_assertFS((uintptr_t)name < VARNAME__MAX || lj_tab_getstr(fs->kt, name) != NULL,
...
static MSize gola_new(LexState* ls, GCstr* name, uint8_t info, BCPos pc)
{
    ...
    lj_assertFS(name == NAME_BREAK || lj_tab_getstr(fs->kt, name) != NULL, "unanchored label name");
    /* NOBARRIER: name is anchored in fs->kt and ls->vstack is not a GCobj. */
    setgcref(ls->vstack[vtop].name, obj2gco(name));
...
static void gola_close(LexState* ls, VarInfo* vg)
{
    ...
    lj_assertFS(bc_op(*ip) == BC_JMP || bc_op(*ip) == BC_UCLO, "bad bytecode op %d", bc_op(*ip));
    setbc_a(ip, vg->slot);
    if (bc_op(*ip) == BC_JMP) {
...
static void fs_fixup_bc(FuncState* fs, GCproto* pt, BCIns* bc, MSize n)
{
    ...
    pt->sizebc = n;
    if (fs->ls->fr2 != LJ_FR2)
        op = BC_NOT; /* Mark non-native prototype. */
    else if ((fs->flags & PROTO_VARARG))
        op = BC_FUNCV;
    else
        op = BC_FUNCF;
...
static GCproto* fs_finish(LexState* ls, BCLine line)
{
    ...
    *(uint32_t*)((char*)pt + ofsk - sizeof(GCRef) * (fs->nkgc + 1)) = 0;
    ...
    lj_vmevent_send(G(L), BC, setprotoV(V, V->top++, pt););
...
static void parse_chunk(LexState* ls)
{
    ...
    while (!islast && !parse_isend(ls->tok)) {
        ...
        lex_opt(ls, ';');
        lj_assertLS(ls->fs->framesize >= ls->fs->freereg && ls->fs->freereg >= ls->fs->nactvar,
                    "bad regalloc");
