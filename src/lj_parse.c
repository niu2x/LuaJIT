...
static void jmp_patchval(
    FuncState* fs, BCPos list, BCPos vtarget, BCReg reg, BCPos dtarget)
{
    while (list != NO_JMP) {
...
static BCPos bcemit_INS(FuncState* fs, BCIns ins)
{
    ...
    if (LJ_UNLIKELY(pc >= fs->bclim)) {
        ...
        lj_mem_growvec(
            fs->L, ls->bcstack, ls->sizebcstack, LJ_MAX_BCINS, BCInsLine);
...
static void bcemit_nil(FuncState* fs, BCReg from, BCReg n)
{
    ...
    bcemit_INS(fs,
        n == 1 ? BCINS_AD(BC_KPRI, from, VKNIL)
...
static void bcemit_method(FuncState* fs, ExpDesc* e, ExpDesc* key)
{
    ...
    func = fs->freereg;
    bcemit_AD(
        fs, BC_MOV, func + 1 + LJ_FR2, obj); /* Copy object to 1st argument. */
...
static void bcemit_arith(FuncState* fs, BinOpr opr, ExpDesc* e1, ExpDesc* e2)
{
    ...
    if (opr == OPR_POW) {
        ...
        lj_assertFS(
            expr_isnumk(e1) || e1->k == VNONRELOC, "bad expr type %d", e1->k);
        expr_toval(fs, e1);
...
static void bcemit_comp(FuncState* fs, BinOpr opr, ExpDesc* e1, ExpDesc* e2)
{
    ...
    if (opr == OPR_EQ || opr == OPR_NE) {
        ...
        switch (e2->k) {
            ...
            case VKSTR:
                ins = BCINS_AD(
                    op + (BC_ISEQS - BC_ISEQV), ra, const_str(fs, e2));
                break;
            case VKNUM:
                ins = BCINS_AD(
                    op + (BC_ISEQN - BC_ISEQV), ra, const_num(fs, e2));
                break;
            default:
...
static void var_new(LexState* ls, BCReg n, GCstr* name)
{
    ...
    if (LJ_UNLIKELY(vtop >= ls->sizevstack)) {
        ...
        lj_mem_growvec(
            ls->L, ls->vstack, ls->sizevstack, LJ_MAX_VSTACK, VarInfo);
    }
    lj_assertFS(
        (uintptr_t)name < VARNAME__MAX || lj_tab_getstr(fs->kt, name) != NULL,
    ...
    for (; vg < vl; vg++)
        if (gcrefeq(vg->name, vl->name) && gola_isgoto(vg)) {
            if (vg->slot < vl->slot) {
                ...
                lj_assertLS(
                    (uintptr_t)name >= VARNAME__MAX, "expected goto name");
...
static void fs_fixup_bc(FuncState* fs, GCproto* pt, BCIns* bc, MSize n)
{
    ...
    pt->sizebc = n;
    bc[0] = BCINS_AD(
        (fs->flags & PROTO_VARARG) ? BC_FUNCV : BC_FUNCF, fs->framesize, 0);
    for (i = 1; i < n; i++)
        bc[i] = base[i].ins;
}
...
static void fs_fixup_line(
    FuncState* fs, GCproto* pt, void* lineinfo, BCLine numline)
{
...
static GCproto* fs_finish(LexState* ls, BCLine line)
{
    ...
    sizept
        = sizeof(GCproto) + fs->pc * sizeof(BCIns) + fs->nkgc * sizeof(GCRef);
        ...
        if (expr_isk(&key) && key.k != VKNIL
            ...
            if (expr_isk_nojump(
                ...
                expr_kvalue(fs, v, &val);
                        ...
                        lj_tab_newkey). */
                settabV(
                    fs->L, v, t); /* Preserve key with table itself as value. */
    ...
    if (!t) { /* Construct TNEW RD: hhhhhaaaaaaaaaaa. */
        ...
        if (fixt) { /* Fix value for dummy keys in template table. */
            ...
            for (i = 0; i <= hmask; i++) {
                ...
                if (tvistab(&n->val)) {
                    lj_assertFS(
                        tabV(&n->val) == t, "bad dummy key in template table");
...
static void parse_constV(LexState* ls, ExpDesc* v)
{
    lj_lex_next(ls); /* Skip 'return'. */
    lex_check(ls, '(');
    GCstr* name = lex_str(ls);

    cTValue* value = lj_tab_getstr(ls->L->consts, name);
    if (!value) {
        lj_lex_error(ls, 0, LJ_ERR_UNDEFINED_CONST, strdata(name));
    }
    expr_init(v, VKNUM, 0);
    copyTV(ls->L, &v->u.nval, value);
}
...
static void expr_simple(LexState* ls, ExpDesc* v)
{
    switch (ls->tok) {
        case TK_number:
            expr_init(
                v, (LJ_HASFFI && tviscdata(&ls->tokval)) ? VKCDATA : VKNUM, 0);
        ...
        case TK_dots: { /* Vararg. */
            ...
            base = fs->freereg - 1;
            expr_init(
                v, VCALL, bcemit_ABC(fs, BC_VARG, base, 2, fs->numparams));
            v->u.s.aux = base;
            break;
        }
...
static void parse_defconst(LexState* ls)
{
    lj_lex_next(ls); /* Skip 'return'. */
    lex_check(ls, '(');
    GCstr* name = lex_str(ls);
    fixstring(name);
    TValue* slot = lj_tab_setstr(ls->L, ls->L->consts, name);
    lex_check(ls, ',');
    if (ls->tok != TK_number)
        err_token(ls, TK_number);
    TValue* value = &ls->tokval;
    copyTV(ls->L, slot, value);
    lj_lex_next(ls);
    lex_check(ls, ')');
    printf("defconst %s\n", strdata(name));
}
    ...
    if (parse_isend(ls->tok) || ls->tok == ';') { /* Bare return. */
        ...
        if (nret == 1) { /* Return one result. */
            if (e.k == VCALL) { /* Check for tail call. */
                ...
                fs->pc--;
                ins = BCINS_AD(
                    bc_op(*ip) - BC_CALL + BC_CALLT, bc_a(*ip), bc_c(*ip));
