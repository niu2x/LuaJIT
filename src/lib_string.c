...
LJLIB_ASM(string_byte) 
LJLIB_REC(string_range 0)
{
...
LJLIB_ASM(string_sub) 
LJLIB_REC(string_range 1)
{
    lj_lib_checkstr(L, 1);
    lj_lib_checkint(L, 2);
...
static int str_find_aux(lua_State* L, int find)
{
    ...
    if (find
        && ((L->base + 3 < L->top && tvistruecond(L->base + 3))
