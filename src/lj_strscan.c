                              ...
                              uint32_t       dig)
{
    ...
    switch (fmt) {
        case STRSCAN_INT:
            if (!(opt & STRSCAN_OPT_TONUM) && x < 0x80000000u + neg && !(x == 0 && neg)) {
...
/* Parse octal number. */
static StrScanFmt
strscan_oct(const uint8_t* p, TValue* o, StrScanFmt fmt, int32_t neg, uint32_t dig)
{
    uint64_t x = 0;
...
/* Parse binary number. */
static StrScanFmt strscan_bin(const uint8_t* p,
                              TValue*        o,
                              StrScanFmt     fmt,
                              uint32_t       opt,
                              int32_t        ex2,
